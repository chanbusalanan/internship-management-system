<x-layouts.app :page-title="'Announcements'">
    <div class="space-y-6">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600">Stay updated with the latest news and announcements.</p>
        </div>
            ...
            @if ($announcements->isNotEmpty())
                <ul class="divide-y divide-gray-200">
                    @foreach ($announcements as $announcement)
                        <li>
                            <a href="{{ route('supervisor.announcements.show', $announcement) }}" class="block px-6 py-4 hover:bg-gray-50 transition">
                                <div class="flex items-center justify-between gap-4">
                                    ...
                                    <div class="text-right shrink-0">
                                        <p class="text-xs text-gray-500">{{ $announcement->postedBy?->full_name ?? 'System' }}</p>
                                        <p class="text-xs text-gray-400 mt-0.5">{{ $announcement->created_at->format('M d, Y') }}</p>
                                    </div>
                                </div>
                            </a>
                        </li>
