<x-layouts.app :page-title="'Log Details'">
    <div class="space-y-6">
        <div class="flex items-center gap-4">
            <a href="{{ route('supervisor.logs.index') }}" class="p-2 rounded-lg text-gray-500 hover:bg-gray-100 transition">
            ...
            <div>
                ...
                <p class="mt-1 text-sm text-gray-600">{{ $log->assignment?->intern?->user?->full_name ?? 'Intern' }} — {{ $log->log_date->format('l, M d, Y') }}</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            ...
            @if ($log->status === 'Pending')
                <div class="mt-6 flex gap-3">
                    <form action="{{ route('supervisor.logs.approve', $log) }}" method="POST">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-700 transition">
                    ...
                    <form action="{{ route('supervisor.logs.reject', $log) }}" method="POST">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="inline-flex items-center gap-2 rounded-lg bg-red-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-red-700 transition">
