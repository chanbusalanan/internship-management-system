<x-layouts.app :page-title="'Documents'">
    <div class="space-y-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Documents</h1>
            <p class="mt-1 text-sm text-gray-600">Review and manage submitted documents.</p>
        </div>

        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <form action="{{ route('hr.submissions.index') }}" method="GET" class="flex items-center gap-3">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse ($submissions as $submission)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $submission->application?->intern?->user?->full_name ?? 'N/A' }}</td>
