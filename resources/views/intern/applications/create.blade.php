<x-layouts.app :page-title="'Submit Application'">
    <div class="space-y-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Submit Application</h1>
                <p class="mt-1 text-sm text-gray-600">Start your internship application process.</p>
            </div>
        </div>

        <div class="bg-white rounded-xl border border-gray-200 p-6">
            <form action="{{ route('intern.applications.store') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                    <label for="application_date" class="block text-sm font-medium text-gray-700 mb-1">Application Date</label>
                    <input type="date" name="application_date" id="application_date" value="{{ old('application_date', now()->format('Y-m-d')) }}" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-sky-500 focus:ring-sky-500 sm:text-sm" required>
                    <p class="mt-1 text-xs text-gray-500">Select the date of your internship application.</p>
                </div>
